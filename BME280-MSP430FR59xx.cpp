...
Distributed as-is; no warranty is given.
******************************************************************************/
// See BME280-MSP430FR59xx.h for additional topology notes.

#include <math.h>
#include "BME280-MSP430FR59xx.h"
...
BME280::BME280(uint8_t csPort, uint8_t csPin)
{
    //Construct with these default settings

    settings.commInterface = SPI_MODE; //Default to SPI
    settings.csPort = csPort;
...
{
    // TODO: implement a function that delays while taking into account differing frequencies.
    // This current application assumes frequency is at 1MHz.
    _delay_cycles(2000);  // Make sure sensor had enough time to turn on. BME280 requires 2ms to start up.
...
void BME280::readFloatPressureFromBurst(uint8_t buffer[], BME280_SensorMeasurements *measurements)
{
    ...
    var1 = ((var1 * var1 * (int64_t)calibration.dig_P3)>>8) + ((var1 * (int64_t)calibration.dig_P2)<<12);
    var1 = (((((int64_t)1)<<47)+var1))*((int64_t)calibration.dig_P1)>>33;
    if (var1 == 0) measurements->pressure = 0; // avoid exception caused by division by zero
    else {
        p_acc = 1048576 - adc_P;
        p_acc = (((p_acc<<31) - var2)*3125)/var1;
        var1 = (((int64_t)calibration.dig_P9) * (p_acc>>13) * (p_acc>>13)) >> 25;
...
float BME280::getReferencePressure(void)
{
...
void BME280::readRegisterRegion(uint8_t *outputPointer, uint8_t offset, uint8_t length)
{
    ...
    GPIO_setOutputLowOnPin(settings.csPort, settings.csPin);
    // Send the device the register you want to read
    SPI_A3_sendRegisterReceiveData(offset | 0x80, outputPointer, length); //Ored with "read request" bit
...
void BME280::writeRegister(uint8_t offset, uint8_t dataToWrite)
{
    // Take the chip select low to select the device
    GPIO_setOutputLowOnPin(settings.csPort, settings.csPin);
    // Send the device the register you want to write to
    SPI_A3_sendByte(offset & 0x7F);
    // Send the data
    SPI_A3_sendByte(dataToWrite);
    // Take the chip select high to de-select
    GPIO_setOutputHighOnPin(settings.csPort, settings.csPin);
}
